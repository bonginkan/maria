...
// Intelligent Router functionality temporarily disabled during Phase 3 testing
// import { getIntelligentRouter } from '../intelligent-_router/app/IntelligentRouterService';
// import {
//   NaturalLanguageProcessor,
//   ProcessedInput,
// } from '../intelligent-_router/infra/NaturalLanguageProcessor';

// Mock implementations for testing phase
...
export class ModeRecognitionEngine extends EventEmitter {
  ...
  private recognitionCache: Map<
    string,
    { result: ModeRecognitionResult; timestamp: number }
  > = new Map();
  ...
  async recognizeMode(
    ...
    try {
      ...
      const cached = this.recognitionCache.get(_cacheKey);
      if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
        return cached.result;
      }

      // Process natural language input
      const _processedInput = await this.nlpProcessor.process(
        context.userInput,
        context.language,
      );

      // Score all modes
      const _modeScores = await this.scoreAllModes(context, _processedInput);

      // Find the best match
      const _bestMatch = this.findBestMatch(_modeScores);
      ...
      const result: ModeRecognitionResult = {
        mode: _bestMatch.mode,
        confidence: _bestMatch.confidence,
        reasoning: this.generateReasoning(_bestMatch, context),
        alternatives: _modeScores
          .filter((_score) => _score.mode.id !== _bestMatch.mode.id)
        ...
        triggers: _bestMatch.triggeredBy.map((type) => ({
          type,
          score: _bestMatch.scores[type] || 0,
          details: this.getTriggerDetails(type, _bestMatch, context),
        })),
      };

      // Cache the result
      this.recognitionCache.set(_cacheKey, {
        result,
        timestamp: Date.now(),
      });
      ...
      if (_processingTime > this.config.recognitionTimeout) {
        console.warn(
          `Mode recognition took ${_processingTime}ms (target: ${this.config.recognitionTimeout}ms)`,
        );
      }
      ...
      console._error("Mode recognition _error:", _error);
      return null;
    }
  }
  ...
  async updateFromFeedback(
    ...
    // For _now, we'll just log the feedback for future improvements
    console.log(
  ...
  private async scoreAllModes(
    ...
    for (const mode of allModes) {
      const score = await this.scoreMode(mode, context, _processedInput);
  ...
  private async scoreMode(
    ...
    for (const trigger of mode.triggers) {
      ...
      switch (trigger.type) {
        case "intent":
          triggerScore = await this.scoreIntentTrigger(
            trigger,
            _processedInput,
            context,
          );
      ...
      if (triggerScore > 0) {
        ...
        if (triggerScore >= trigger.confidence) {
          ...
          reasoning.push(
            `${trigger.type} trigger activated (_score: ${triggerScore.toFixed(2)})`,
          );
        }
      }
    }
  ...
  private async scoreIntentTrigger(
    ...
    for (const condition of trigger.conditions) {
      ...
      switch (condition.field) {
        case "keywords":
          conditionScore = this.scoreKeywordCondition(
            condition,
            _processedInput,
          );
          break;
        case "entities":
          conditionScore = this.scoreEntityCondition(
            condition,
            _processedInput,
          );
          break;
  ...
  private async scoreContextTrigger(
    trigger: ModeTrigger,
    context: ModeContext,
  ): Promise<number> {
    let score = 0;
    ...
    for (const condition of trigger.conditions) {
      ...
      switch (condition.field) {
        ...
        case "previousModes":
          ...
          break;
        case "commandHistory":
          conditionScore = this.scoreCommandHistoryCondition(
            condition,
            context,
          );
          break;
        case "defaultMode":
          conditionScore = condition.value === "true" ? 0.5 : 0; // Low _score for default
          break;
      }
  ...
  private async scoreSituationTrigger(
    trigger: ModeTrigger,
    context: ModeContext,
  ): Promise<number> {
    let score = 0;
    ...
    for (const condition of trigger.conditions) {
      ...
      switch (condition.field) {
        case "errorState":
          ...
          break;
        case "projectContext":
          conditionScore = this.scoreProjectContextCondition(
            condition,
            context,
          );
          break;
  ...
  private async scorePatternTrigger(
    _trigger: ModeTrigger,
    context: ModeContext,
  ): Promise<number> {
    ...
    for (const pattern of context.userPatterns) {
      if (pattern.sequence.length > 0) {
        ...
        const recentUsage =
          (Date.now() - pattern.lastUsed.getTime()) / (1000 * 60 * 60); // hours
  ...
  private scoreKeywordCondition(
    ...
    for (const keyword of keywords) {
      if (
        _processedInput.keywords.includes(keyword.toLowerCase()) ||
  ...
  private scoreEntityCondition(
    ...
    for (const entityType of entities) {
      if (_processedInput.entities.some((e) => e.type === entityType)) {
        matches++;
      }
    }
  ...
  private async scoreIntentCondition(
    ...
    // Intelligent Router temporarily disabled during Phase 3 testing
    // try {
    //   const _router = getIntelligentRouter();
    //   const _intent = await router.route(context.userInput);

    //   if (_intent && intent.command) {
    ...
    // Simple fallback matching during testing phase
    const inputLower = context.userInput.toLowerCase();
    const conditionValue = condition.value.toString().toLowerCase();

    return inputLower.includes(conditionValue) ? 0.8 : 0;
  }

  private scoreCurrentModeCondition(
    condition: TriggerCondition,
    context: ModeContext,
  ): number {
  ...
  private scorePreviousModeCondition(
    condition: TriggerCondition,
    context: ModeContext,
  ): number {
    const targetModes = Array.isArray(condition.value)
      ? condition.value
      : [condition.value];
    const recentModeIds = context.previousModes
      .slice(0, 3)
      .map((m) => m.mode.id);

    return targetModes.some((mode: string) => recentModeIds.includes(mode))
      ? 0.8
      : 0;
  }

  private scoreCommandHistoryCondition(
    condition: TriggerCondition,
    context: ModeContext,
  ): number {
    const targetCommands = Array.isArray(condition.value)
      ? condition.value
      : [condition.value];
    const recentCommands = context.commandHistory.slice(-5);

    return targetCommands.some((cmd: string) => recentCommands.includes(cmd))
      ? 0.7
      : 0;
  }

  private scoreErrorStateCondition(
    condition: TriggerCondition,
    context: ModeContext,
  ): number {
  ...
  private scoreProjectContextCondition(
    condition: TriggerCondition,
    context: ModeContext,
  ): number {
    ...
    switch (condition.field) {
      ...
      case "hasErrors":
        return condition.value === context.projectContext.hasErrors.toString()
          ? 0.9
          : 0;
      case "hasTests":
        return condition.value === context.projectContext.hasTests.toString()
          ? 0.6
          : 0;
      default:
        return 0;
  ...
  private findBestMatch(_modeScores: ModeScore[]): ModeScore | null {
    if (modeScores.length === 0) {
      return null;
    }

    const bestScore = _modeScores[0];
  ...
  private generateReasoning(
    ...
    if (modeScore.scores.intent > 0.5) {
      reasons.push(
        `Strong intent match (${(modeScore.scores.intent * 100).toFixed(0)}%)`,
      );
    }

    if (modeScore.scores._context > 0.5) {
      reasons.push(
        `Context alignment (${(modeScore.scores._context * 100).toFixed(0)}%)`,
      );
    }

    if (modeScore.scores.situation > 0.5) {
      reasons.push(
        `Situational factors (${(modeScore.scores.situation * 100).toFixed(0)}%)`,
      );
    }

    if (modeScore.scores.pattern > 0.3) {
      reasons.push(
        `User pattern match (${(modeScore.scores.pattern * 100).toFixed(0)}%)`,
      );
    }
  ...
  private generateCacheKey(context: ModeContext): string {
    const keyParts = [
      ...
      context.errorState ? "_error" : "normal",
    ];
