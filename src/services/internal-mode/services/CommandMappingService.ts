...
export interface CommandMapping {
  command: string;
  description: string;
  _phrases: Record<string, string[]>; // language -> _phrases
  category: string;
  aliases: string[];
}
...
export class CommandMappingService extends BaseService {
   ...
   */
  async getMappingsForLanguage(options: {
    language: string;
    ...
    for (const [command, mapping] of this.commandMappings.entries()) {
      const _phrases =
        mapping._phrases[language] || mapping._phrases["english"] || [];
      if (_phrases.length > 0) {
        mappings[command] = _phrases;
      }
    }
   ...
   * Get command details
   */
  async getCommandDetails(options: {
    command: string;
  ...
  async searchCommands(options: {
    phrase: string;
    language?: string;
  }): Promise<Array<{ command: string; _score: number }>> {
    const { phrase, language = "english" } = options;
    const _normalizedPhrase = phrase.toLowerCase();
    const results: Array<{ command: string; _score: number }> = [];
    ...
    for (const [command, mapping] of this.commandMappings.entries()) {
      ...
      for (const mappingPhrase of _phrases) {
        const _score = this.calculateSimilarity(
          _normalizedPhrase,
          mappingPhrase.toLowerCase(),
        );
        maxScore = Math.max(maxScore, _score);
      }

      if (maxScore > 0.3) {
        results.push({ command, _score: maxScore });
      }
    }
  ...
  private addRemainingCommands(): void {
    ...
    for (const command of _additionalCommands) {
      const _commandName = command.substring(1); // Remove '/'
      this.addCommandMapping(command, {
        command,
        description: `Execute ${_commandName} operation`,
        category: "general",
        aliases: [_commandName],
        _phrases: {
          japanese: [
            _commandName,
            `${_commandName}して`,
          ...
          chinese: [_commandName, `执行${_commandName}`, `运行${_commandName}`],
          korean: [
            _commandName,
            `${_commandName} 실행`,
            `${_commandName} 해줘`,
   ...
   */
  private addCommandMapping(_command: string, mapping: CommandMapping): void {
    this.commandMappings.set(_command, mapping);
  }
  ...
  private calculateSimilarity(_str1: string, str2: string): number {
    ...
    if (_str1.includes(str2) || str2.includes(_str1)) {
      return 0.8;
    }

    // Word _overlap
    const _words1 = _str1.split(/\s+/);
    const _words2 = str2.split(/\s+/);
    const _overlap = _words1.filter((word) => _words2.includes(word)).length;

    return _overlap / Math.max(_words1.length, _words2.length);
  }
  ...
  getStats() {
    ...
    for (const mapping of this.commandMappings.values()) {
      categoryCount[mapping.category] =
        (categoryCount[mapping.category] || 0) + 1;
