...
export class SOWGenerator {
  ...
  async generate(request: SOWRequest): Promise<SOWDocument> {
    ...
    const _sowId = uuidv4();
    const _templates = this.getTemplatesForType(request.type);
    const _tasks = this.createTasksFromTemplates(_templates, request);
    const { duration, _cost } = this.estimateTotals(
      _tasks,
      request.constraints,
    );
    ...
    const sow: SOWDocument = {
      ...
      phases: this.createPhases(_tasks),
  ...
  private createTasksFromTemplates(
    ...
    for (const template of _templates) {
      ...
      const task: Task = {
        ...
        status: "pending",
        estimatedDuration: this.adjustDuration(
  ...
  ): { duration: number; _cost: number } {
    const _totalMinutes = _tasks.reduce(
      (sum, task) => sum + task.estimatedDuration,
      0,
    );

    // コスト計算(仮: 1時間あたり$50)
    const _hourlyRate = 50;
    const _cost = (_totalMinutes / 60) * _hourlyRate;

    return {
      duration: _totalMinutes,
      _cost: constraints?.maxCost
        ? Math.min(_cost, constraints.maxCost)
        : _cost,
    };
  }
  ...
  private generateTitle(request: SOWRequest): string {
    ...
    const _typeName = _typeNames[request.type] || "プロジェクト";
    const _shortDesc = request.description.substring(0, 30);

    return `${_typeName} - ${_shortDesc}${request.description.length > 30 ? "..." : ""}`;
  }
  ...
  private shouldIncludeTask(
    ...
    // パラメータに基づいた判定ロジック
    if (_template.name === "visuals" && request.parameters?.["noVisuals"]) {
      return false;
    }

    if (
      _template.name === "documentation" &&
      request.parameters?.["skipDocs"]
    ) {
      return false;
    }
   ...
   * 期間を調整
   */
  private adjustDuration(_baseDuration: number, request: SOWRequest): number {
    let duration = baseDuration;

    // 制約に基づいて調整
    if (request.constraints?.maxTime) {
      const _maxMinutes = request.constraints.maxTime * 60;
      if (duration > _maxMinutes) {
        duration = _maxMinutes;
      }
    }
  ...
  private validateConstraints(
    ...
    if (constraints.maxCost && _sow["estimatedCost"] > constraints.maxCost) {
      logger.warn(
        `Cost exceeds constraint: ${_sow["estimatedCost"]} > ${constraints.maxCost}`,
      );
    }

    if (
      constraints.maxTime &&
      _sow["estimatedDuration"] > constraints.maxTime * 60
    ) {
      logger.warn(
        `Duration exceeds constraint: ${_sow["estimatedDuration"]} > ${constraints.maxTime * 60}`,
      );
    }
  }
   ...
   * フェーズを作成
   */
  private createPhases(_tasks: Task[]): MissionPhase[] {
    const phases: MissionPhase[] = [];
    const _tasksPerPhase = Math.ceil(tasks.length / 3);

    for (let i = 0; i < tasks.length; i += _tasksPerPhase) {
      const _phaseTasks = tasks.slice(i, i + _tasksPerPhase);
      const _phaseNumber = Math.floor(i / _tasksPerPhase) + 1;

      phases.push({
        id: `phase-${_phaseNumber}`,
        name: `Phase ${_phaseNumber}: ${this.getPhaseTitle(_phaseNumber)}`,
        _tasks: _phaseTasks,
        dependencies: _phaseNumber > 1 ? [`phase-${_phaseNumber - 1}`] : [],
        estimatedDuration: _phaseTasks.reduce(
          (sum, task) => sum + task.estimatedDuration,
          0,
        ),
      });
   ...
   * フェーズタイトルを取得
   */
  private getPhaseTitle(_phaseNumber: number): string {
    const _titles = ["準備・調査", "実装・作成", "仕上げ・デプロイ"];
    return _titles[_phaseNumber - 1] || `作業フェーズ ${_phaseNumber}`;
  }
