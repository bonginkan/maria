...
export class SOWGenerator {
   ...
   */
  public async generate(_intent: IntentAnalysis, _request: string): Promise<SOW> {
    const _template = this.getTemplate(intent.primaryIntent);
    const _tasks = this.generateTasks(intent, request);
    const _deliverables = this.generateDeliverables(intent, tasks);
    const _timeline = this.generateTimeline(tasks, deliverables);
    const _risks = this.generateRisks(intent);
   ...
   */
  public async createFromDecomposition(_decomposition: TaskDecomposition): Promise<SOW> {
    const _deliverables = this.extractDeliverablesFromTasks(decomposition.subtasks);
   ...
   */
  private generateTitle(_intent: IntentAnalysis, _request: string): string {
    const _action = this.getActionFromIntent(intent.primaryIntent);
   ...
   */
  private generateObjective(_intent: IntentAnalysis, _request: string): string {
    const _complexity = intent.estimatedComplexity;
    const _approach = intent.suggestedApproach;

    const _objective = `To ${this.getActionFromIntent(intent.primaryIntent).toLowerCase()} `;
  ...
  private generateTasks(_intent: IntentAnalysis, _request: string): Task[] {
    ...
    const _implementationTasks = this.generateImplementationTasks(intent, baseTime);
    implementationTasks.forEach((_task) => {
    ...
    if (intent.secondaryIntents.includes('testing_required')) {
      tasks.push(
        this.createTask(
          ...
          'high',
          baseTime * 0.15,
          implementationTasks.map((_t) => t.id),
        ),
      );
  ...
  private generateDeliverables(_intent: IntentAnalysis, _tasks: Task[]): Deliverable[] {
    ...
    deliverables.push({
      ...
      estimatedEffort: tasks.reduce((sum, _t) => sum + (t.estimatedTime || 0), 0) / 60,
    });
   ...
   * Generate timeline
   */
  private generateTimeline(_tasks: Task[], _deliverables: Deliverable[]): Timeline {
    const _totalHours = tasks.reduce((sum, _t) => sum + (t.estimatedTime || 0), 0) / 60;
    ...
    const milestones: Milestone[] = [
      ...
      },
      {
        ...
        date: endDate,
        deliverables: deliverables.map((_d) => d.id),
        status: 'pending',
      },
    ];
    ...
    const phases: Phase[] = [
      {
        ...
        tasks: tasks.slice(0, 2).map((_t) => t.id),
        status: 'not_started',
      },
      {
        id: crypto.randomUUID(),
        ...
        tasks: tasks.slice(2, -2).map((_t) => t.id),
        status: 'not_started',
      },
      {
        id: crypto.randomUUID(),
        ...
        endDate,
        tasks: tasks.slice(-2).map((_t) => t.id),
        status: 'not_started',
      },
    ];
  ...
  private getTemplate(_intent: string): SOWTemplate {
  ...
  private extractSubject(_request: string): string {
    // Simple extraction - in production, use NLP
    const _words = request.split(' ').slice(0, 10);
    return words.join(' ');
  }

  private formatSecondaryIntent(_intent: string): string {
    return intent.replace(/_/g, ' ').replace(/\b\w/g, (_c) => c.toUpperCase());
  }

  private formatRequirement(_requirement: string): string {
    return requirement.replace(/_/g, ' ').replace(/\b\w/g, (_c) => c.toUpperCase());
  }

  private formatRisk(_riskId: string): string {
    return riskId.replace(/_/g, ' ').replace(/\b\w/g, (_c) => c.toUpperCase());
  }
  ...
  private generateTimelineFromTasks(_tasks: Task[]): Timeline {
    const _totalTime = tasks.reduce((sum, _t) => sum + (t.estimatedTime || 0), 0);
    const _startDate = new Date();
    const _endDate = new Date(startDate.getTime() + totalTime * 60 * 1000);
