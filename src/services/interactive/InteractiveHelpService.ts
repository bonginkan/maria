...
export class InteractiveHelpService extends EventEmitter {
  ...
  public getDisplayInfo(): {
    ...
    _selectedCommand: CommandInfo | null;
    _categories: string[];
    ...
    return {
      ...
      _categories: this.state.categories,
  ...
  private extractCategories(commands: CommandInfo[]): string[] {
    ...
    return Array.from(_categories).sort();
  }
  ...
  private updateFilteredCommands(): void {
    ...
    if (this.state.selectedCategory !== "all") {
      filtered = filtered.filter(
        (cmd) => cmd.category === this.state.selectedCategory,
      );
    }
  ...
  private moveSelection(delta: number): void {
    const _newIndex = this.state.selectedIndex + delta;
    const _maxIndex = this.state.filteredCommands.length - 1;

    if (_newIndex < 0) {
      this.state.selectedIndex = _maxIndex; // 循環: 最初から最後へ
    } else if (_newIndex > _maxIndex) {
      this.state.selectedIndex = 0; // 循環: 最後から最初へ
    } else {
      this.state.selectedIndex = _newIndex;
    }
  ...
  private previousCategory(): void {
    const _currentIndex = this.state.categories.indexOf(
      this.state.selectedCategory,
    );
    const _newIndex =
      _currentIndex > 0 ? _currentIndex - 1 : this.state.categories.length - 1;

    this.state.selectedCategory = this.state.categories[_newIndex] || "all";
    this.updateFilteredCommands();
    this.state.selectedIndex = 0;
  ...
  private nextCategory(): void {
    ...
    const _newIndex =
      _currentIndex < this.state.categories.length - 1 ? _currentIndex + 1 : 0;
  ...
  private selectCurrentCommand(): void {
    const _selectedCommand =
      this.state.filteredCommands[this.state.selectedIndex];
    if (!_selectedCommand) {
      return;
    }

    // 使用頻度を更新
    this.frequencyTracker.recordUsage(_selectedCommand.name);

    this.emit("commandSelected", _selectedCommand);
    this.stop();
  }
  ...
  private setupEventListeners(): void {
    // キーボードナビゲーションイベント
    this.navigationHandler.on(
      "keyPressed",
      (_key: string, modifiers: string[]) => {
        this.handleKeyPress(_key, modifiers);
      },
    );

    // 検索エンジンイベント
    this.searchEngine.on("searchCompleted", (_results: CommandInfo[]) => {
      this.emit("searchResults", _results);
    });

    // 頻度トラッカーイベント
    this.frequencyTracker.on(
      "frequencyUpdated",
      (_commandName: string, frequency: number) => {
        this.emit("frequencyUpdated", _commandName, frequency);
      },
    );
  }
   ...
   * キープレスを処理
   */
  private handleKeyPress(_key: string, modifiers: string[]): void {
    if (!this.state.isActive) {
      return;
    }

    const _hasCtrl = modifiers.includes("ctrl");
    const _hasShift = modifiers.includes("shift");
