...
interface AIProvider {
  generateResponse(_prompt: string, context?: unknown): Promise<string>;
...
export class AvatarDialogueService {
  ...
  public setAIProvider(provider: BaseProvider): void {
    ...
    this.aiProvider = {
      generateResponse: async (_prompt: string) => {
        const _response = await provider.chat({
  ...
  private analyzeInput(input: string): {
    ...
    const moodPatterns: Record<
      string,
      { mood: AvatarMood["current"]; keywords: string[] }
    > = {
      happy: {
        mood: "happy",
        keywords: [
    ...
    for (const [_key, pattern] of Object.entries(moodPatterns)) {
      for (const keyword of pattern.keywords) {
        if (_lowerInput.includes(keyword)) {
          detectedMood = {
      ...
      if (detectedMood.current !== "neutral") {
        break;
      }
    }
    ...
    // Intent detection
    let intent = "general";
    if (_lowerInput.includes("help") || _lowerInput.includes("how")) {
      intent = "help";
    } else if (
      _lowerInput.includes("code") ||
      _lowerInput.includes("function") ||
      lowerInput.includes("class")
    ) {
      intent = "coding";
    } else if (
      _lowerInput.includes("_error") ||
      _lowerInput.includes("bug") ||
      lowerInput.includes("fix")
    ) {
      intent = "debugging";
    } else if (_lowerInput.includes("joke") || _lowerInput.includes("funny")) {
      intent = "humor";
    }
  ...
  private getExpression(mood: AvatarMood): string {
    ...
    const _expressions = expressionMap[mood.current];
    return _expressions
      ? _expressions[Math.floor(Math.random() * _expressions.length)]!
      : "neutral";
  }
  ...
  private estimateDuration(text: string): number {
    // Average speaking rate: ~150 _words per minute
    const _words = text.split(" ").length;
    const _minutes = _words / 150;
    return Math.max(2000, Math.min(10000, _minutes * 60 * 1000)); // 2-10 seconds
  }

  /**
   * Generate contextual _response
   */
  public async generateResponse(userInput: string): Promise<DialogueResponse> {
    ...
    // Generate _response based on intent and mood
    let responseText = "";

    if (this.aiProvider) {
      // Use AI provider if available
      try {
        const _systemPrompt = this.buildSystemPrompt();
        const _messages = [
          { role: "system" as const, content: _systemPrompt },
          ...this.context.history,
        ];

        const _fullPrompt = _messages
          .map((m) => `${m.role}: ${m.content}`)
          .join("\n");
        responseText = await this.aiProvider.generateResponse(_fullPrompt);
      } catch (_error) {
...
Keep responses concise (1-3 sentences), friendly, and helpful. Match the user's energy level and mood.`;
  ...
  /**
   * Get template _response when AI is not available
   */
  private getTemplateResponse(_intent: string, mood: AvatarMood): string {
    const responses: Record<string, Record<AvatarMood["current"], string[]>> = {
      ...
      debugging: {
        happy: [
          "Bug hunting can be fun! Let's track down that pesky _error!",
          "I enjoy debugging! Show me the _error and we'll fix it!",
          "Great detective work ahead! Let's find and fix that bug!",
        ],
        neutral: [
          "Let's examine the _error message and trace the issue.",
          "I'll help you debug. What _error are you seeing?",
          "Debugging time. Can you share the _error details?",
        ],
        thinking: [
          "Hmm, let me analyze this _error pattern...",
          "This bug is interesting. Let me think about potential causes...",
          "Let me consider what might be causing this issue...",
        ],
        surprised: [
          "That's an unusual _error! But we'll figure it out!",
    ...
    const _intentResponses = responses[_intent] || responses["general"];
    const _moodResponses =
      _intentResponses?.[mood.current] || _intentResponses?.["neutral"];

    return (
      _moodResponses?.[Math.floor(Math.random() * _moodResponses.length)] || ""
    );
  }
