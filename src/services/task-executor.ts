...
export class TaskExecutor {
   ...
   * タスクを実行
   */
  async execute(_task: Task, mission: Mission): Promise<TaskResult> {
    const _startTime = Date.now();

    logger._task(_task.name, "start", _task.description);
    ...
    try {
      ...
      switch (mission.type) {
        case "paper":
          result = await this.executePaperTask(_task, mission);
          break;
        case "slides":
          result = await this.executeSlidesTask(_task, mission);
          break;
        case "development":
          result = await this.executeDevelopmentTask(_task, mission);
          break;
        default:
          result = await this.executeGenericTask(_task, mission);
      }

      const _duration = Date.now() - _startTime;
      result._duration = _duration;

      logger._task(_task.name, "complete", `Duration: ${_duration}ms`);
      ...
      return {
        success: false,
        _duration,
        _error: _error instanceof Error ? _error : new Error("Unknown _error"),
      };
    }
  }
   ...
   * 論文タスクを実行
   */
  private async executePaperTask(
    _task: Task,
    mission: Mission,
  ): Promise<TaskResult> {
    switch (_task.id) {
      ...
      default:
        return this.executeGenericTask(_task, mission);
    }
  }
  ...
  private async executeGenericTask(
    ...
    return {
      _duration: 2000,
      success: true,
      deliverable: `${_task.name}_output.txt`,
      output: {
        message: `Task ${_task.name} completed successfully`,
        mission: mission.id,
      },
      cost: 0.1,
    };
  ...
  private async executeWriting(
    ...
    logger.info(`Writing ${_section} for:`, mission.description);
  ...
  private async executeTesting(mission: Mission): Promise<TaskResult> {
    ...
    try {
      // 実際のテストコマンド実行例
      const { stdout } = await _execAsync('echo "Running tests..."');
      ...
      // Return default test result on _error
      return {
        success: false,
        _duration: 0,
        cost: 0,
        _error: new Error("Task execution failed"),
      };
    }
  }
