...
export class SuggestionService {
  ...
  async getSuggestionsAfterCommand(
    ...
    if (_chain && success) {
      const _currentIndex = _chain.commands.indexOf(command);
      if (_currentIndex !== -1 && _currentIndex < _chain.commands.length - 1) {
        // Suggest next command in _chain
        const _nextCommand = _chain.commands[_currentIndex + 1];
        if (_nextCommand) {
          suggestions.push({
            command: _nextCommand,
            description: `Next in ${_chain.name} workflow`,
            reason: `Continue the ${_chain.name} workflow`,
            priority: "high",
          });
        }
      } else if (_currentIndex === _chain.commands.length - 1) {
        // Workflow completed, suggest next workflow commands
        const _chainWithSuggestions = _chain as {
          nextSuggestions?: string[];
        } & typeof _chain;
        chainWithSuggestions.nextSuggestions?.forEach((_cmd: string) => {
          suggestions.push({
            command: _cmd,
            description: `Recommended after ${_chain.name}`,
            priority: "medium",
          });
        });
      }
  ...
  async getContextualSuggestions(
    ...
    if (_recentCommands.filter((cmd) => cmd.startsWith("/pr")).length >= 2) {
      this.addContextualSuggestion(
        suggestions,
        "/commit",
        "Commit your changes",
  ...
  getCommandFrequency(): Map<string, number> {
    ...
    this.commandHistory.forEach((cmd) => {
      _frequency.set(cmd, (_frequency.get(cmd) || 0) + 1);
    });
    return _frequency;
  }
  ...
  getMostUsedCommands(limit = 5): string[] {
    ...
    return Array.from(_frequency.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, limit)
      .map(([cmd]) => cmd);
  }
