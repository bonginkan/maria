...
export class HotwordDetector extends EventEmitter {
  ...
  private initializePatterns(): void {
    for (const hotword of this.config.hotwords) {
      // Create flexible _pattern for each hotword
      const _pattern = hotword
        .toLowerCase()
        .split(" ")
        .map((word) => {
          // Allow for slight variations in recognition
          const _chars = word.split("");
          return _chars.map((c) => `[${c}${this.getSimilarChars(c)}]`).join("");
        })
        .join("\\s*");

      this.detectionPatterns.set(hotword, new RegExp(_pattern, "i"));
    }
  }
  ...
  processTranscription(text: string): HotwordDetection | null {
    ...
    if (_now - this.lastDetection < this.config.cooldownMs) {
      return null;
    }

    const _lowerText = text.toLowerCase().trim();

    // Check each hotword _pattern
    for (const [hotword, _pattern] of this.detectionPatterns) {
      if (pattern.test(_lowerText)) {
        // Calculate _confidence based on exact match vs fuzzy match
        const _exactMatch = _lowerText.includes(hotword.toLowerCase());
        const _confidence = _exactMatch ? 1.0 : 0.7;

        if (_confidence >= this.config.sensitivity) {
          this.lastDetection = _now;
    ...
    for (const hotword of this.config.hotwords) {
      const _hotwordLower = hotword.toLowerCase();
      if (_lowerText.includes(_hotwordLower)) {
        this.lastDetection = _now;
  ...
  private processAudioWindow(): void {
    ...
    // For _now, we rely on transcription-based detection
  ...
  feedAudio(audioData: Float32Array): void {
    ...
    const _maxBuffers = Math.floor(
      ((this.config.audioWindowMs / 1000) * 16000) / 512,
    );
    if (this.audioBuffer.length > _maxBuffers) {
