...
export const BRAND_COLORS = {
  ...
  BRANDPRIMARY: chalk.magenta,
  BRANDSECONDARY: chalk.magentaBright,
...
// 相互作用カラー(ユーザー操作)
export const _INTERACTION_COLORS = {
  // リンク・コマンド
...
export const LAYOUT_COLORS = {
  // ボーダー・区切り
  BORDERPRIMARY: chalk.magenta, // ブランドボーダー
  BORDERSECONDARY: chalk.gray, // 通常ボーダー
  SEPARATOR: chalk.dim.gray, // 区切り線

  // 背景・強調
  BACKGROUNDSUBTLE: chalk.bgGray,
  HIGHLIGHTBG: chalk.bgBlue,
} as const;
...
export class ColorPalette {
   ...
   * セマンティックカラー取得
   */
  static semantic(_type: keyof typeof SEMANTIC_COLORS) {
    return SEMANTIC_COLORS[_type];
  }

  /**
   * テキスト階層カラー取得
   */
  static text(_level: keyof typeof TEXT_HIERARCHY) {
    return TEXT_HIERARCHY[_level];
  }

  /**
   * インタラクションカラー取得
   */
  static interaction(_state: keyof typeof _INTERACTION_COLORS) {
    return _INTERACTION_COLORS[_state];
  }

  /**
   * レイアウトカラー取得
   */
  static layout(_element: keyof typeof LAYOUT_COLORS) {
    return LAYOUT_COLORS[_element];
  }

  /**
   * ブランドカラー取得
   */
  static brand(_variant: keyof typeof BRAND_COLORS) {
    return BRAND_COLORS[_variant];
  }

  /**
   * ステータス別カラー(アイコンと組み合わせ用)
   */
  static _status(_status: "success" | "error" | "warning" | "info") {
    const _colorMap = {
  ...
  static validateContrast(
    ...
    // ここでは基本的なルールで判定
    const _highContrast = ["white", "yellow", "cyan", "green"];

    const _isHighContrast = _highContrast.some(
      (color) => foreground.includes(color) || background.includes("black"),
    );

    return {
      isValid: _isHighContrast,
      ratio: _isHighContrast ? 4.5 : 2.0,
      level: _isHighContrast ? "AA" : "FAIL",
    };
  }
  ...
  static showPalette(): void {
    ...
    Object.entries(SEMANTIC_COLORS).forEach(([name, colorFn]) => {
      console.log(
        `  ${colorFn(name.padEnd(10))} ${this.text("CAPTION")(name)}`,
      );
    });
...
export type InteractionColorName = keyof typeof _INTERACTION_COLORS;
