...
import {
  OptimizedAnimations,
  _TransitionEffects,
} from "../animations/OptimizedAnimations.js";
import {
  StatusBarRenderer,
  SystemStatus,
} from "../components/StatusBarRenderer.js";
...
import {
  ...
  TEXT_HIERARCHY,
  UNIFIED_COLORS,
} from "../optimized-design-system.js";
...
async function showProgressBars() {
  ...
  const _steps = ["Initialize", "Configure", "Build", "Deploy", "Complete"];
  for (let i = 1; i <= _steps.length; i++) {
    console.clear();
    OptimizedProgress.renderSteps(i, _steps.length, _steps, { compact: false });
    await delay(500);
  }
  console.log("\n");
  ...
  OptimizedProgress.renderMultiTask(_tasks);
  console.log("\n");
}
...
async function showTables() {
  ...
  OptimizedTable.render(_data, undefined, {
    border: "light",
  ...
  OptimizedTable.render(_data.slice(0, 2), undefined, {
    border: "heavy",
    alignment: "center",
  });
  console.log("\n");
  ...
  OptimizedTable.renderVertical(_data[0]);
  console.log("\n");

  // Grid layout
  console.log(TEXT_HIERARCHY.CAPTION("Grid Layout:"));
  const _items = ["React", "Vue", "Angular", "Svelte", "Next.js", "Nuxt"];
  OptimizedTable.renderGrid(_items, 3);
  console.log("\n");
}
...
async function showAnimations() {
  ...
  const _spinner = OptimizedAnimations._spinner("Processing request");
  await delay(2000);
  ...
  const _dots = OptimizedAnimations.loadingDots("Connecting to server");
  await delay(2000);
  dots.stop(true);
  console.log("\n");
  ...
  const _wave = OptimizedAnimations.waveAnimation("Analyzing code");
  await delay(2000);
  wave.stop(true);
  console.log("\n");
...
function showComplete() {
  ...
  console.log("\n");
  OptimizedComponents.renderStatus(
    "success",
    "Thank you for exploring MARIA CODE!",
  );
  console.log("\n");
}
