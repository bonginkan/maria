...
const ProjectManager: React.FC<ProjectManagerProps> = ({ action, config, onUpdate, onExit, args }) => {
...
`;
  ...
  const handleInit = useCallback(async () => {
    try {
      ...
      setError(`Failed to create MARIA.md: ${err}`);
    }
  }, [createMariaMdTemplate]);

  const confirmOverwrite = useCallback(async (confirmed: boolean) => {
    if (confirmed) {
      try {
        const mariaMdPath = join(process.cwd(), 'MARIA.md');
        const mariaMdContent = createMariaMdTemplate();
        ...
        setError(`Failed to update MARIA.md: ${err}`);
      }
    } else {
  ...
  const handleAddDir = useCallback(() => {
    ...
    const updatedConfig = {
      ...
      project: {
        ...config.project,
        workingDirectories: [
          ...(config.project?.workingDirectories || []),
          resolvedPath
        ]
      }
    };
  ...
  const exportConversation = useCallback(async () => {
    try {
      ...
      const conversationData = {
        ...
        messages: [
          { role: 'user', content: 'Example conversation message', timestamp: new Date().toISOString() },
          { role: 'assistant', content: 'Example AI response', timestamp: new Date().toISOString() }
        ]
      };
...
${conversationData.messages.map(m => `### ${m.role.toUpperCase()}
${m.content}
*${m.timestamp}*`).join('\n\n')}`;
    ...
    return (
      <Box flexDirection="column">
        ...
        <Box marginTop={1}>
          <SelectInput
            items={confirmItems}
            onSelect={(item) => confirmOverwrite(item.value)}
          />
        </Box>
      </Box>
    );
