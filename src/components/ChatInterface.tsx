...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ mode, projectPath }) => {
  ...
  const handleInput = useCallback(async (value: string) => {
    ...
    try {
      ...
      if (value.toLowerCase().includes('auto pilot software') || value.toLowerCase().includes('autopilot software')) {
...
I've prepared a detailed SOW and requirements document for your review.`;
        ...
        sowData = {
          ...
          deliverables: [
            ...
            { name: 'Requirements Specification', description: 'Detailed technical requirements' },
          ...
          tasks: [
            ...
            },
            {
              ...
              description: 'Build the core AI engine for requirements analysis and code generation',
              duration: '4 weeks',
              required: true,
...
Would you like me to create a detailed plan for your specific needs?`;
  ...
  const handleSpecialCommand = useCallback(async (command: string) => {
    ...
    const supportedCommands = ['/init', '/add-dir', '/memory', '/export', '/agents', '/mcp', '/ide', '/install-github-app'];
...
• Time: ${new Date().toLocaleTimeString()}`;
  ...
  const handleStepConfirmation = useCallback(async (confirmedSteps: string[]) => {
    if (!currentSOW) return;

    // Update SOW with selected steps
    const updatedSOW = {
      ...currentSOW,
      tasks: currentSOW.tasks?.filter((task: any, index: number) => 
  ...
  return (
    <Box flexDirection="column" padding={1}>
      <Box marginBottom={1} justifyContent="space-between">
        <Box>
          <Text bold color="magenta">MARIA CODE Chat</Text>
          <Text> - {mode} mode</Text>
        </Box>
        <Box>
          <Text dimColor>
            Context: {contextStats.messagesInWindow} msgs | {contextStats.totalTokens}/{contextStats.maxTokens} tokens ({Math.round(contextStats.usagePercentage)}%)
