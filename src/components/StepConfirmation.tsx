...
const StepConfirmation: React.FC<StepConfirmationProps> = ({ steps, onConfirm, onCancel }) => {
        ...
        {steps.map((step, i) => (
          <Box key={step.id} marginBottom={1}>
            ...
            <Box marginLeft={3}>
              ...
              {step.estimatedTime && <Text color="gray">Estimated time: {step.estimatedTime}</Text>}
  ...
  const renderSelecting = () => (
    <Box flexDirection="column">
      ...
      <Box marginTop={1} flexDirection="column">
        <Text color="cyan">
          A - Select all • R - Reset to required • ENTER to confirm • ESC to cancel
        </Text>
        <Text color="gray">
          Selected: {selectedSteps.size}/{steps.length} steps
        </Text>
      </Box>
    </Box>
  );
    ...
    return (
      <Box flexDirection="column">
        ...
        {totalEstimatedTime > 0 && (
          <Box marginTop={1}>
            <Text color="yellow">Total estimated time: ~{totalEstimatedTime} minutes</Text>
          </Box>
        )}
