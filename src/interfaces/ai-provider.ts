...
export interface AIProvider {
  ...
  chat(_messages: Message[], options?: ChatOptions): Promise<AIResponse>;
  stream(
    _messages: Message[],
  ...
  generateCode?(_prompt: string, options?: CodeOptions): Promise<CodeResponse>;
  embeddings?(
    text: string | string[],
  ...
  // Cost and usage
  estimateCost?(_tokens: number, operation?: "input" | "output"): number;
...
export interface AIProviderFactory {
  create(_type: string, config?: ProviderConfig): AIProvider;
  register(
    _type: string,
...
export class ModelNotFoundError extends AIProviderError {
  constructor(_modelId: string, provider?: string) {
    super(`Model ${_modelId} not found`, "MODEL_NOT_FOUND", provider, false);
  }
}

export class RateLimitError extends AIProviderError {
  constructor(_provider: string, retryAfter?: number) {
    super(
...
export class AuthenticationError extends AIProviderError {
  constructor(_provider: string) {
    super("Authentication failed", "AUTH_FAILED", _provider, false);
  }
}
...
export function hasCodeCapability(
  ...
  return (
    provider.capabilities.code && typeof provider.generateCode === "function"
  );
}
...
export abstract class BaseAIProvider implements AIProvider {
  ...
  abstract readonly _type: "cloud" | "local";
  ...
  abstract getModelInfo(_modelId?: string): Promise<ModelInfo>;
