...
import { ReadyCommandsService, CommandSearchResult, CategoryInfo } from "../../../../services/help/ReadyCommandsService";
import { ReadyCommand } from "../../../../types/CommandReadiness";

export class DynamicHelpCommand extends BaseCommand {
  readonly meta: CommandMeta = {
    name: "help",
    category: "core",
    description: "Display dynamic help for available commands"
  };

  name = "help";
  category = "core" as const;
  _description = "📚 Display help for contract-validated READY commands only";
  override _aliases = ["h", "?"];
  ...
  override examples: CommandExample[] = [
    {
      input: "/help",
      _description: "Show READY commands organized by category",
      output: "Contract-validated commands with performance info",
    },
    {
      input: "/help code",
      _description: "Show detailed help for the code command",
      output: "Usage, examples, and contract info for /code",
    },
    {
      input: "/help --category core", 
      _description: "Show all READY commands in core category",
  ...
  private async showGeneralHelp(): Promise<CommandResult> {
    ...
    return this.success(lines.join("\n"), {
      ...
      totalCommands: stats.totalReady,
      categories: categories.map(c => c.name),
      avgPerformance: stats.avgResponseTime
    });
  }
  ...
  private async showCommandHelp(commandName: string): Promise<CommandResult> {
    ...
    return this.success(lines, {
      ...
      performance: command.contract.maxResponseTime
    });
  }
  ...
  private async showCategoryHelp(categoryName: string): Promise<CommandResult> {
    ...
    if (commands.length === 0) {
      ...
      return this.error(
        `No READY commands in category: ${categoryName}`,
        "CATEGORY_NOT_FOUND",
        {
          availableCategories,
          tip: "Only categories with READY commands are shown"
        }
      );
    }
  ...
  private async searchCommands(searchTerm: string): Promise<CommandResult> {
    ...
    if (searchResults.length === 0) {
      return this.error(
        ...
        "NO_SEARCH_RESULTS",
        {
          tip: "Try a different search term or /help --quickstart for essential commands"
        }
      );
    }
  ...
  private async showQuickStart(): Promise<CommandResult> {
    ...
    return this.success(lines.join("\n"), {
      type: "quickstart",
      essentialCommands: quickCommands.map(c => c.name)
    });
  }
  ...
  /**
   * Validation for dynamic help command
   */
  override async validate(
    ...
    if (options["category"]) {
      try {
        ...
        if (!availableCategories.includes(options["category"] as string)) {
          return {
            success: false,
            error: `Unknown READY category: ${options["category"]}. Available: ${availableCategories.join(", ")}`
          };
        }
      } catch (error) {
        return {
          success: false,
          error: "Failed to load READY commands for validation"
        };
      }
    }
