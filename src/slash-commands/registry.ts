...
export class CommandRegistry {
  ...
  unregister(name: string): boolean {
    ...
    if (command.cleanup) {
      command
        .cleanup()
        .catch((err) =>
          logger.error(`Error cleaning up command ${name}:`, err),
        );
    }
  ...
  async execute(
    ...
    try {
      ...
      const result = await this.runMiddlewareChain(
        command,
        parsedArgs,
        context,
        async () => {
  ...
  private setupDefaultMiddlewares(): void {
    ...
    this.registerMiddleware({
      name: "error-handler",
      priority: 1,
      async execute(command, _args, _context, next) {
        try {
          return await next();
        } catch (innerError) {
  ...
  private checkRateLimit(
    ...
    const commandLimits =
      this.rateLimits.get(command.name) || new Map<string, RateLimitEntry>();
  ...
  private async runMiddlewareChain(
    ...
    const names = command.middleware || [];
