...
export class HelpCommand extends BaseCommand {
  ...
  private async searchHelp(searchTerm: string): Promise<SlashCommandResult> {
    ...
    if (matchingTopics.length === 0 && matchingCommands.length === 0) {
      return {
        success: false,
        message:
          `No help content found for "${searchTerm}".\n\n` +
    ...
    return [
      {
        ...
        category: 'basics',
        content:
          `${chalk.bold('Welcome to MARIA CODE!')} 🎉\n\n` +
...

      {
        ...
        category: 'reference',
        content:
          `MARIA provides 40+ slash commands organized into categories:\n\n` +
        ...
        examples: ['/help <command-name>', '/help --category=development', '/help --search=git'],
...

      {
        ...
        content:
          `MARIA supports multiple AI providers and models:\n\n` +
...

      {
        ...
        category: 'configuration',
        content:
          `MARIA can be extensively customized to match your workflow:\n\n` +
  ...
  private getAllCommands() {
    return [
      // Core commands
      {
        ...
        examples: ['/code "create a REST API"', '/code "add error handling"'],
      },
      {
        name: 'test',
      ...
      {
        name: 'deploy',
        description: 'Deploy application with guidance',
        category: 'project',
        usage: '/deploy [platform]',
        examples: ['/deploy vercel'],
      },
      {
        name: 'memory',
        description: 'Manage project memory and context',
        category: 'project',
        usage: '/memory [view|edit|clear]',
        examples: ['/memory view'],
      },
      ...
      },
      {
        ...
        category: 'configuration',
        usage: '/setup [quick|advanced|reset]',
      ...
      },
      {
        ...
        category: 'configuration',
        usage: '/permissions [list|grant|revoke]',
        examples: ['/permissions list'],
      },
  ...
  private getSpecificCommandHelp(commandName: string): string | null {
    const specificHelp: Record<string, string> = {
      code:
        `${chalk.bold('Code Generation Tips:')}\n` +
      ...
      settings:
        `${chalk.bold('Settings Categories:')}\n` +
