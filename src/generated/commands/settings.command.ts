...
export class SettingsCommand extends BaseCommand {
  ...
  private async editSettings(category?: string, args: string[] = [], flags: Record<string, unknown> = {}): Promise<SlashCommandResult> {
    if (!category) {
      return {
        success: false,
        message: 'Usage: /settings edit <category> [setting] [value] or use /settings wizard for guided editing'
      };
    }
    ...
    return {
      success: true,
      message: `✅ Updated ${category}.${settingKey}: ${currentValue} → ${newValue}\n\n` +
               `${chalk.blue('💡 Tip:')} Use \`/settings view ${category}\` to see all settings in this category.`
    };
  }

  private async resetSettings(category?: string, flags: Record<string, unknown> = {}): Promise<SlashCommandResult> {
    if (!flags.confirm) {
      return {
        success: false,
  ...
  private async restoreSettings(backupFile?: string): Promise<SlashCommandResult> {
    if (!backupFile) {
      ...
      try {
        ...
        const backups = files.filter(f => f.startsWith('settings-backup-') && f.endsWith('.json'));
    ...
    try {
      ...
      for (const [category, categorySettings] of Object.entries(settings)) {
        if (category in defaults) {
          const defaultCategory = defaults[category as keyof SettingsConfig] as Record<string, unknown>;
  ...
  private async manageEnvironment(template?: string, args: string[] = [], flags: Record<string, unknown> = {}): Promise<SlashCommandResult> {
    try {
      ...
      switch (action.toLowerCase()) {
        ...
        default:
          return {
            success: false,
            message: `Unknown environment action: ${action}\n\n${chalk.bold('Available actions:')}\n` +
  ...
  private async viewEnvironmentVariables(): Promise<SlashCommandResult> {
    try {
      ...
      const categories = {
        ai: ['OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GOOGLE_AI_API_KEY', 'GROQ_API_KEY', 'GROK_API_KEY'],
        firebase: ['NEXT_PUBLIC_FIREBASE_API_KEY', 'NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN', 'FIREBASE_ADMIN_CLIENT_EMAIL'],
      ...
      const otherVars = envLines.filter(line => {
        const varName = line.split('=')[0];
        return !Object.values(categories).flat().some(key => varName.includes(key));
      });
  ...
  private async generateEnvironmentTemplate(providerType: string, flags: Record<string, unknown>): Promise<SlashCommandResult> {
    const templates = {
      ai: {
        title: 'AI Providers',
        vars: {
    ...
    try {
      ...
      if (envExists && !flags.overwrite) {
        return {
          success: false,
          message: `${chalk.red('⚠️  .env.local already exists')}\n\n` +
...
`;
      ...
      return {
        success: true,
        message: `${chalk.green('✅ Sample file created!')}\n\n` +
  ...
  private async editEnvironmentFile(): Promise<SlashCommandResult> {
    ...
    return {
      success: true,
      message: `${chalk.blue('📝 Edit Environment Variables')}\n\n` +
