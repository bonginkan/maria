...
export class HooksCommand extends BaseCommand {
  ...
  private async listHooks(args: string[]): Promise<SlashCommandResult> {
    ...
    const formatHook = (hook: Hook) => {
      ...
      const cmd = chalk.gray(hook.command.substring(0, 50) + (hook.command.length > 50 ? '...' : ''));
  ...
  private async addHook(args: string[]): Promise<SlashCommandResult> {
    if (args.length < 2) {
      return {
        success: false,
        message: 'Usage: /hooks add <event> "<command>" [--name="hook-name"] [--description="desc"]'
      };
    }
    ...
    const validEvents: HookEvent[] = [
      ...
      'pre-build', 'post-build', 'pre-test', 'post-test',
      'on-error', 'on-success', 'file-change', 'startup', 'shutdown'
    ];
    ...
    return {
      success: true,
      message: `✅ Hook '${hookName}' added successfully!\n` +
               `Event: ${event}\n` +
  ...
  private async removeHook(args: string[]): Promise<SlashCommandResult> {
    ...
    const hookIndex = config.hooks.findIndex(h => 
      h.name === identifier || h.id === identifier
    );
  ...
  private async toggleHook(identifier: string, enabled: boolean): Promise<SlashCommandResult> {
    ...
    const hook = config.hooks.find(h => 
      h.name === identifier || h.id === identifier
    );
  ...
  private async showLogs(args: string[]): Promise<SlashCommandResult> {
    try {
      ...
      const formatLog = (log: { timestamp: string; hookName: string; event: string; status: string; duration: number; output: string }) => {
  ...
  private async showStatus(): Promise<SlashCommandResult> {
    ...
    const eventCounts = config.hooks.reduce((acc, hook) => {
  ...
  private async initializeHooks(): Promise<SlashCommandResult> {
    ...
    return {
      success: true,
      message: `✅ Hooks system initialized with ${defaultHooks.length} default hooks!\n` +
  ...
  private async importHooks(args: string[]): Promise<SlashCommandResult> {
    ...
    try {
      ...
      return {
        success: true,
        message: `✅ Hooks configuration imported from ${args[0]}\n` +
