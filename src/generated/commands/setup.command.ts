...
export class SetupCommand extends BaseCommand {
  ...
  private async detectExistingTools(): Promise<Array<{ name: string; configPath: string }>> {
    ...
    const toolPaths = [
      { name: 'GitHub Copilot', path: path.join(homeDir, '.vscode', 'extensions'), pattern: '*copilot*' },
  ...
  private async showSetupStatus(): Promise<SlashCommandResult> {
    ...
    const completionPercentage = Math.round((setupState.completedSteps.length / profile.steps.length) * 100);
  ...
  private async resetSetup(flags: Record<string, unknown>): Promise<SlashCommandResult> {
    ...
    try {
      ...
      return {
        success: true,
        message: '✅ Setup state reset. You can now run a fresh setup with /setup quick or /setup advanced'
      };
    } catch {
      return {
        success: true,
  ...
  private async validateSetup(): Promise<SlashCommandResult> {
    ...
    try {
      ...
      validations.push({ item: 'Configuration file', status: 'pass', message: 'Found .maria/config.json' });
    } catch {
      validations.push({ item: 'Configuration file', status: 'fail', message: 'Missing configuration file' });
    }
    ...
    try {
      ...
      validations.push({ item: 'Git configuration', status: 'pass', message: 'Git user configured' });
    } catch {
      validations.push({ item: 'Git configuration', status: 'warn', message: 'Git user not configured' });
    }
