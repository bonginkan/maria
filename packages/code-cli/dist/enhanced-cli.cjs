...
async function writeConfig(config2, path4) {
  return new Promise((resolve, reject) => {
    try {
      saveConfig(config2, path4);
      resolve();
    } catch (error) {
      reject(error);
    }
...
function saveConfig(config2, path4) {
  const configPath = path4 || (0, import_path.join)(process.cwd(), CONFIG_FILE);
  const lines = [];
  if (config2.user) {
    lines.push("[user]");
...
var TemplateManager = class _TemplateManager {
  ...
  async deleteTemplate(id) {
    ...
    try {
      ...
      fs4.unlinkSync((0, import_path2.join)(this.templatesDir, `${id}.json`));
    } catch (error) {
...
function loadEnvironmentVariables() {
  const localEnvPath = (0, import_path4.join)(process.cwd(), ".env.local");
  if ((0, import_fs4.existsSync)(localEnvPath)) {
  ...
  const envPath = (0, import_path4.join)(process.cwd(), ".env");
  ...
  const lmstudioEnvPath = (0, import_path4.join)(process.cwd(), ".env.lmstudio");
  if ((0, import_fs4.existsSync)(lmstudioEnvPath)) {
