...
var ChatDisplay = class {
  ...
  displayAssistantResponse(content) {
    ...
    if (content.includes("[LINT ERRORS") || content.includes("[TYPESCRIPT ERRORS")) {
      console.log(chalk.red.bold("\u{1F50D} Error Analysis Mode"));
    } else if (content.includes("[ATTACHED IMAGES")) {
      console.log(chalk.cyan.bold("\u{1F5BC}\uFE0F  Image Analysis Mode"));
    } else if (content.includes("[URL RESEARCH")) {
      console.log(chalk.blue.bold("\u{1F52C} Research Mode"));
    }
    console.log();
    this.messages.push({
      role: "assistant",
    ...
    lines.forEach((line) => {
      if (line.startsWith("### ")) {
        ...
        const formatted = line.replace(
          /([^\s]+\.[a-z]+):(\d+):(\d+)/g,
          (_, file, line2, col) => chalk.cyan(file) + ":" + chalk.yellow(line2) + ":" + chalk.yellow(col)
        );
        console.log(formatted);
  ...
  displayEnhancedProgress(message, context) {
    console.log(chalk.cyan(`\u{1F504} ${message}`));
    if (context) {
  ...
  displayQuickActions(actions) {
    ...
    console.log("\n" + chalk.bold.yellow("\u{1F4A1} Quick Actions:"));
...
\u{1F517} Starting command chain${options.chainName ? `: ${options.chainName}` : ""}`));
    if (options.chainDescription) {
      console.log(chalk2.gray(`   ${options.chainDescription}`));
    }
    console.log(chalk2.gray(`   Commands: ${commands2.join(" \u2192 ")}
`));
    try {
      for (const command of commands2) {
        ...
        if (options.interactive) {
          ...
          if (!shouldExecute) {
            console.log(chalk2.yellow(`\u23ED\uFE0F  Skipping ${command}`));
            continue;
          }
        }
...
\u25B6\uFE0F  Executing: ${command} ${args.join(" ")}`));
        try {
          ...
          results.push(result);
          if (result.success) {
            console.log(chalk2.green(`\u2705 ${command} completed successfully`));
            if (result.message) {
              console.log(chalk2.gray(this.truncateMessage(result.message)));
            }
          } else {
            console.log(chalk2.red(`\u274C ${command} failed`));
            console.log(chalk2.red(result.message));
            errors.push({ command, error: result.message });
          }
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : "Unknown error";
          console.log(chalk2.red(`\u274C ${command} threw an error: ${errorMessage}`));
...
\u{1F3C1} Chain execution completed`));
  ...
  generateSummary(executed, planned, errors, success) {
    let summary = "\n";
    if (success) {
      summary += chalk2.green(`\u2728 All commands executed successfully!
`);
    } else {
      summary += chalk2.yellow(`\u26A0\uFE0F  Chain completed with errors
`);
    }
    summary += chalk2.gray(`   Executed: ${executed.length}/${planned.length} commands
`);
    if (executed.length < planned.length) {
      const skipped = planned.slice(executed.length);
      summary += chalk2.gray(`   Skipped: ${skipped.join(", ")}
`);
    }
    if (errors.length > 0) {
      summary += chalk2.red(`   Errors: ${errors.length}
`);
      errors.forEach((err) => {
        summary += chalk2.red(`     - ${err.command}: ${err.error}
`);
      });
    }
    return summary;
...
var BatchExecutionEngine = class _BatchExecutionEngine {
  ...
  async executeBatch(commands2, context, options = {}) {
    ...
    if (options.dryRun) {
      console.log(chalk3.yellow("DRY RUN MODE - Commands will not be executed\n"));
      commands2.forEach((cmd, i) => {
        console.log(chalk3.gray(`${i + 1}. ${cmd.command} ${cmd.args.join(" ")}`));
        if (cmd.condition) console.log(chalk3.gray(`   IF: ${cmd.condition}`));
      });
  ...
  async executeSingleCommand(cmd, context, result) {
    if (cmd.condition && !this.evaluateCondition(cmd.condition)) {
      result.skipped++;
      console.log(chalk3.gray(`\u23ED\uFE0F  Skipping ${cmd.command} (condition not met)`));
      return;
    }
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (cmdResult.success) {
          result.succeeded++;
          console.log(chalk3.green(`\u2705 Success (${duration}ms)`));
          result.results.push({
          ...
          if (attempts < maxAttempts) {
            console.log(chalk3.yellow(`\u26A0\uFE0F  Failed, retrying (${attempts}/${maxAttempts})...`));
            await this.delay(1e3);
          } else {
            result.failed++;
            console.log(chalk3.red(`\u274C Failed: ${cmdResult.message}`));
            result.results.push({
              command: `${cmd.command} ${cmd.args.join(" ")}`,
              success: false,
        ...
        const duration = Date.now() - startTime;
        if (attempts < maxAttempts) {
          console.log(chalk3.yellow(`\u26A0\uFE0F  Error, retrying (${attempts}/${maxAttempts})...`));
          await this.delay(1e3);
        } else {
          result.failed++;
          result.executed++;
          const errorMsg = error instanceof Error ? error.message : "Unknown error";
          console.log(chalk3.red(`\u274C Error: ${errorMsg}`));
          result.results.push({
            command: `${cmd.command} ${cmd.args.join(" ")}`,
            success: false,
            error: errorMsg,
...
\u26A1 Executing ${commands2.length} commands in parallel`));
  ...
  printSummary(result) {
    ...
    console.log(chalk3.green(`Succeeded: ${result.succeeded}`));
    if (result.failed > 0) {
      console.log(chalk3.red(`Failed: ${result.failed}`));
    }
    if (result.skipped > 0) {
      console.log(chalk3.gray(`Skipped: ${result.skipped}`));
    }
...
var HotkeyManager = class _HotkeyManager {
  ...
  getHelpText() {
    ...
    let help = chalk4.bold("\nAvailable Hotkeys:\n\n");
    bindings.forEach((binding) => {
      const hotkey = chalk4.cyan(this.formatHotkey(binding));
      const command = chalk4.yellow(binding.command);
      const args = binding.args ? chalk4.gray(` ${binding.args.join(" ")}`) : "";
      const desc = binding.description ? chalk4.gray(` - ${binding.description}`) : "";
      const status = !binding.enabled ? chalk4.red(" [disabled]") : "";
      help += `  ${hotkey.padEnd(20)} \u2192 ${command}${args}${desc}${status}
`;
    });
    help += `
${chalk4.gray("Use /hotkey to manage hotkeys")}
`;
    return help;
  }
  /**
...
function loadEnvironmentVariables() {
  const localEnvPath = join3(process.cwd(), ".env.local");
  if (existsSync3(localEnvPath)) {
  ...
  const lmstudioEnvPath = join3(process.cwd(), ".env.lmstudio");
  if (existsSync3(lmstudioEnvPath)) {
...
var InteractiveModelSelector = class {
  ...
  async initialize() {
    console.log(chalk5.cyan("\u{1F50D} Checking available AI models...\n"));
    ...
    if (this.lmStudioStatus === "not-running") {
      console.log(chalk5.yellow("\u26A0\uFE0F  LM Studio is not running. Starting it now...\n"));
  ...
  async checkLMStudio() {
    try {
      ...
      console.error(chalk5.red("Error checking LM Studio:"), error);
      this.lmStudioStatus = "not-installed";
    }
  }
  async startLMStudio() {
    try {
      console.log(chalk5.cyan("\u{1F680} Starting LM Studio server..."));
      ...
      if (this.lmStudioStatus === "running") {
        console.log(chalk5.green("\u2705 LM Studio server started successfully!\n"));
      ...
      console.error(chalk5.red("Failed to start LM Studio:"), error);
      return false;
    }
  }
  async selectModel() {
        ...
        if (key.name === "up") {
          ...
          if (selected.type === "local" && !selected.available) {
            if (this.lmStudioStatus === "not-running") {
              ...
              if (!started) {
                console.log(chalk5.red("\u274C Failed to start LM Studio"));
                this.cleanup();
                resolve(null);
                return;
              }
...
\u2705 Selected: ${selected.name}`));
  ...
  render() {
    console.clear();
    console.log(chalk5.bold.cyan("\u{1F916} Select AI Model"));
    console.log(chalk5.gray("Use \u2191\u2193 arrows to navigate, Enter to select, ESC to cancel\n"));
    const localModels = this.models.filter((m) => m.type === "local");
    const cloudModels = this.models.filter((m) => m.type === "cloud");
    let currentIndex = 0;
    if (localModels.length > 0) {
      console.log(chalk5.bold.green("\u{1F4BB} Local Models (Offline)"));
      localModels.forEach((model) => {
        const isSelected = currentIndex === this.selectedIndex;
        const prefix = isSelected ? chalk5.cyan("\u25B6 ") : "  ";
        const status = model.available ? chalk5.green("\u2713") : chalk5.red("\u2717");
        const line = `${prefix}${status} ${chalk5.bold(model.name)} ${chalk5.gray(`(${model.context}, ${model.vram})`)} - ${chalk5.dim(model.description)}`;
        console.log(line);
        currentIndex++;
      });
      console.log();
    ...
    if (cloudModels.length > 0) {
      console.log(chalk5.bold.blue("\u2601\uFE0F  Cloud Models"));
      cloudModels.forEach((model) => {
        const isSelected = currentIndex === this.selectedIndex;
        const prefix = isSelected ? chalk5.cyan("\u25B6 ") : "  ";
        const status = model.available ? chalk5.green("\u2713") : chalk5.red("\u2717");
        const apiStatus = model.apiKeySet ? "" : chalk5.yellow(" (No API key)");
        const line = `${prefix}${status} ${chalk5.bold(model.name)} ${chalk5.gray(`(${model.context})`)} - ${chalk5.dim(model.description)}${apiStatus}`;
        console.log(line);
        currentIndex++;
      });
    }
    console.log(chalk5.gray("\n\u2500".repeat(60)));
    console.log(chalk5.cyan("Current model: ") + chalk5.yellow(process.env.LMSTUDIO_DEFAULT_MODEL || process.env.AI_MODEL || "None"));
    if (this.lmStudioStatus === "running") {
      console.log(chalk5.green("LM Studio: Running at http://localhost:1234"));
    } else if (this.lmStudioStatus === "not-running") {
      console.log(chalk5.yellow("LM Studio: Not running (will start automatically)"));
    } else if (this.lmStudioStatus === "not-installed") {
      console.log(chalk5.red("LM Studio: Not installed"));
    }
  }
...
\u23F3 Loading ${modelId}...`));
      ...
      console.log(chalk5.green(`\u2705 Model ${modelId} loaded successfully`));
    } catch (error) {
      console.error(chalk5.red(`Failed to load model: ${error}`));
    }
  }
...
var ChatContextService = class _ChatContextService extends EventEmitter {
  ...
  constructor(config2) {
    ...
    this.config = {
      ...
      persistPath: config2?.persistPath || path2.join(process.env.HOME || "", ".maria", "context")
    };
  ...
  async handleHotkey(args) {
    ...
    switch (subCommand) {
      ...
      case "export": {
        ...
        try {
          const fs4 = await import("fs/promises");
          await fs4.writeFile(filename, JSON.stringify(config2, null, 2));
          return {
            success: true,
...
var EnhancedMariaCLI = class {
  ...
  showLogo() {
    const logo = `
${chalk6.bold.magenta("\u2554\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2557")}
${chalk6.bold.magenta("\u2551")}                                                              ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}    ${chalk6.bold.magenta("\u2588\u2588\u2588\u2557   \u2588\u2588\u2588\u2557 \u2588\u2588\u2588\u2588\u2588\u2557 \u2588\u2588\u2588\u2588\u2588\u2588\u2557 \u2588\u2588\u2557 \u2588\u2588\u2588\u2588\u2588\u2557")}                    ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}    ${chalk6.bold.magenta("\u2588\u2588\u2588\u2588\u2557 \u2588\u2588\u2588\u2588\u2551\u2588\u2588\u2554\u2550\u2550\u2588\u2588\u2557\u2588\u2588\u2554\u2550\u2550\u2588\u2588\u2557\u2588\u2588\u2551\u2588\u2588\u2554\u2550\u2550\u2588\u2588\u2557")}                   ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}    ${chalk6.bold.magenta("\u2588\u2588\u2554\u2588\u2588\u2588\u2588\u2554\u2588\u2588\u2551\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2551\u2588\u2588\u2588\u2588\u2588\u2588\u2554\u255D\u2588\u2588\u2551\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2551")}                   ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}    ${chalk6.bold.magenta("\u2588\u2588\u2551\u255A\u2588\u2588\u2554\u255D\u2588\u2588\u2551\u2588\u2588\u2554\u2550\u2550\u2588\u2588\u2551\u2588\u2588\u2554\u2550\u2550\u2588\u2588\u2557\u2588\u2588\u2551\u2588\u2588\u2554\u2550\u2550\u2588\u2588\u2551")}                   ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}    ${chalk6.bold.magenta("\u2588\u2588\u2551 \u255A\u2550\u255D \u2588\u2588\u2551\u2588\u2588\u2551  \u2588\u2588\u2551\u2588\u2588\u2551  \u2588\u2588\u2551\u2588\u2588\u2551\u2588\u2588\u2551  \u2588\u2588\u2551")}                   ${chalk6.bold.magenta("\u2551")}
...
${chalk6.bold.magenta("\u2551")}           ${chalk6.bold.magenta("\u2591\u255A\u2550\u2550\u2550\u2550\u255D\u2591\u2591\u255A\u2550\u2550\u2550\u2550\u255D\u2591\u255A\u2550\u2550\u2550\u2550\u2550\u255D\u2591\u255A\u2550\u2550\u2550\u2550\u2550\u2550\u255D")}                   ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}                                                              ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}          ${chalk6.bold.magenta("AI-Powered Development Platform")}                    ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}              ${chalk6.magenta("v1.0.0 | TypeScript Monorepo")}                     ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}                                                              ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}             ${chalk6.bold.cyan("(c) 2025 Bonginkan Inc.")}                        ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u2551")}                                                              ${chalk6.bold.magenta("\u2551")}
${chalk6.bold.magenta("\u255A\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u255D")}
`;
    console.log(logo);
  ...
  showWelcome() {
    console.log("");
    console.log(chalk6.bold.cyan("Welcome to MARIA CODE CLI!"));
    console.log("");
    console.log(chalk6.yellow("How can I help you today?"));
    console.log("");
    console.log(chalk6.gray("Just type naturally:"));
    console.log(chalk6.gray('  \u2022 "Create a REST API for user management"'));
    console.log(chalk6.gray('  \u2022 "Help me design an auto-pilot software system"'));
    console.log(chalk6.gray('  \u2022 "Debug this TypeScript error"'));
    console.log(chalk6.gray('  \u2022 "What is /init command?"'));
    console.log("");
    console.log(chalk6.dim("Commands: /help, /init, /status | Exit: Ctrl+C"));
  }
  drawInputBox(value = "", isMultiline = false) {
    ...
    const borderColor = chalk6.white;
    const labelText = " Input ";
    const label = chalk6.white.bold(labelText);
    ...
    if (isMultiline) {
      console.log(borderColor("\u2502 ") + chalk6.gray("(Multiline: Shift+Enter for new line, Enter to send, Esc to cancel)") + " ".repeat(Math.max(0, innerWidth - 60)) + borderColor("\u2502"));
      const lines = value.split("\n");
      const maxLines = 5;
      const displayLines = lines.slice(-maxLines);
      displayLines.forEach((line, i) => {
        const displayLine = line.length > innerWidth - 5 ? line.substring(0, innerWidth - 8) + "..." : line;
        const prefix = i === displayLines.length - 1 ? chalk6.yellow("\u27A4 ") : "  ";
      ...
      console.log(borderColor("\u2502 ") + chalk6.yellow("\u27A4 ") + chalk6.white(displayValue) + padding + borderColor("\u2502"));
    }
      ...
      if (key && key.shift && key.name === "return") {
        ...
        if (this.isMultiline) {
          ...
          this.handleCommand(content).catch((err) => {
            console.error(chalk6.red("Command error:"), err);
          });
        } else {
          const content = this.inputBuffer;
          if (content.trim()) {
  ...
  async handleCommand(input) {
    ...
    clearScreenDown(process.stdout);
    console.log(chalk6.cyan("Processing..."));
    ...
    try {
      ...
      this.display.displayAssistantResponse(
        chalk6.red(`\u274C Error: ${error instanceof Error ? error.message : "Unknown error"}`)
      );
    }
    setTimeout(() => {
      this.drawInputBox();
  ...
  **Example requests:**
  ...
  async handleEnhancedInput(input) {
    ...
    try {
      ...
      this.display.displayAssistantResponse(
        chalk6.red(`\u274C Error processing message: ${error instanceof Error ? error.message : "Unknown error"}`)
      );
    }
  }
...
Please try again or use a specific command.`;
  ...
  exit() {
    ...
    console.log(chalk6.magenta("Happy coding!\n"));
    process.exit(0);
  }
};
...
//# sourceMappingURL=enhanced-cli-BJ2DW4XO.js.map