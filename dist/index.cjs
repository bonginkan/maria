...
\u2022 For offline work: Run LM Studio first`;
  ...
  async handleInitCommand() {
    try {
      ...
      const configPath = path2.join(currentDir, ".maria-code.toml");
      const memoryPath = path2.join(currentDir, "MARIA.md");
      if (fs2.existsSync(configPath)) {
        return `\u2705 MARIA project already initialized!

\u{1F4C1} Project: ${path2.basename(currentDir)}
...
[project]
name = "${path2.basename(currentDir)}"
version = "1.0.0"
...
## Project: ${path2.basename(currentDir)}
...
`;
      fs2.writeFileSync(configPath, tomlConfig);
      fs2.writeFileSync(memoryPath, memoryContent);
...
// src/commands/graph.ts
var MARIA_DIR = (0, import_path2.join)((0, import_os2.homedir)(), ".maria-code");
var JWT_FILE = (0, import_path2.join)(MARIA_DIR, "neo4j-jwt.token");
var JWT_EXPIRY_MINUTES = 15;
function ensureMariaDir() {
  if (!(0, import_fs2.existsSync)(MARIA_DIR)) {
    (0, import_fs2.mkdirSync)(MARIA_DIR, { recursive: true });
  }
}
...
function saveJWT(jwt) {
  ensureMariaDir();
  (0, import_fs3.writeFileSync)(JWT_FILE, jwt, { mode: 384 });
  (0, import_fs2.chmodSync)(JWT_FILE, 384);
}
...
`;
    (0, import_fs3.writeFileSync)(outputPath, placeholderContent);
